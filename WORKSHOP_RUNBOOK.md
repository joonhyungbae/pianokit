...
이 문서는 현재 파이프라인(`02_somax_collab.ipynb` -> `03_rave_collab.ipynb` -> `05_stage.ipynb`)을
...
- `20-50분` NB02 실행: Somax 세션 생성 및 `session_report.json` 검증
- `50-80분` NB03 실행: RAVE 1프로파일 우선 변환 (`warm_legato` 권장)
...
강의자는 탭을 전환하며 수업을 진행합니다.

## 3) 트랙 분기 (완주/안전)

- **완주 트랙**
  - NB02 -> NB03(최소 1개, 가능하면 3개 프로파일) -> NB05
- **안전 트랙**
  - NB02 또는 사전 제공 `collab_rave` 산출물 사용 -> NB05 중심 시연
...
- **2단계**: NB02->NB03->NB05를 순서대로 실행하고 산출물 제출
...
- [ ] `02_somax_collab.ipynb`가 최소 1회 실행 완료된 샘플 아티팩트 준비
- [ ] `03_rave_collab.ipynb`용 모델 파일 경로 확인
...
- [ ] NB02 렌더 실패: 사전 제공 Somax WAV로 NB03 진행
- [ ] NB03 실패 지속: 안전 트랙 전환 후 NB05 시연 완주
